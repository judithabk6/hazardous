...
class IncidenceScoreComputer:
    ...
    def log_survival(self, y_true, y_pred, times):
        if (self.event_ids_ > 0).sum() > 1 and self.event_of_interest != "any":
            warnings.warn(
                "Computing the survival Brier score only makes "
                "sense with a binary event indicator or when setting "
        ...
        return self.log_incidence(y_true, 1 - y_pred, times)

    def log_incidence(self, y_true, y_pred, times):
        """ """
        epsilon = 1e-10
        event_true, duration_true = check_y_survival(y_true)
        check_event_of_interest(self.event_of_interest)
        ...
        for t_idx, t in enumerate(times):
            ...
            # XXX: refactor and rename this function to make it possible to
            # also compute the time-dependent binary cross-entropy loss.
            log_error = np.log(np.abs(y_true_binary - y_pred[:, t_idx] + epsilon))
            log_scores[:, t_idx] = weights * log_error

        return log_scores.mean(axis=0)
    ...
    def integrated_log_survival(self, y_true, y_pred, times):
        log_scores = self.log_survival(
            y_true,
            y_pred,
            times,
        )
        return self._time_integrated(log_scores, times)

    def integrated_log_incidence(self, y_true, y_pred, times):
        log_scores = self.log_incidence(
            y_true,
            y_pred,
            times,
        )
...
def log_survival(
    ...
    return computer.log_survival(y_test, y_pred, times)


def integrated_log_survival(
    y_train,
    y_test,
    y_pred,
    times,
    ...
    return computer.integrated_log_survival(y_test, y_pred, times)
...
def log_incidence(
    ...
    return computer.log_incidence(y_test, y_pred, times)


def integrated_log_incidence(
    y_train,
    y_test,
    y_pred,
    times,
    ...
    return computer.integrated_log_incidence(y_test, y_pred, times)
